...
DATA_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/focusflow_ui_execution.XXXXXX")"
LOG_FILE="$(mktemp "${TMPDIR:-/tmp}/focusflow_ui_execution.XXXXXX")"
PID=""
...
usleep(1_500_000)
click(0.83, 0.54)      // +5 in the floating timer preview.
click(0.83, 0.46)      // I am stuck in the floating timer preview.
usleep(1_500_000)
'
}
...
if [[ "${FOCUSFLOW_UI_STRICT_CLICK:-0}" != "1" ]]; then
  echo "FocusFlow execution controls UI smoke launched. Set FOCUSFLOW_UI_STRICT_CLICK=1 to click execution controls."
  exit 0
fi
...
APPLESCRIPT
then
  echo "Named Execution Accessibility click automation could not run. Trying coordinate fallback with CGEvent..." >&2
  set +e
  COORDINATE_RESULT="$(run_coordinate_click_smoke 2>&1)"
  COORDINATE_STATUS=$?
  set -e
  if [[ "$COORDINATE_STATUS" -ne 0 ]]; then
    echo "Execution controls UI smoke could not click controls. Grant Accessibility/Input Monitoring permission and rerun with FOCUSFLOW_UI_STRICT_CLICK=1 for strict failure." >&2
    echo "$COORDINATE_RESULT" >&2
    exit 2
  fi
fi

if ! rg -q '"event_type":"runtimeExtended"|"event_type" : "runtimeExtended"' "$DATA_ROOT" 2>/dev/null; then
  echo "runtimeExtended event was not written." >&2
  exit 2
fi
if ! rg -q '"event_type":"stuckHelpRequested"|"event_type" : "stuckHelpRequested"' "$DATA_ROOT" 2>/dev/null; then
  echo "stuckHelpRequested event was not written." >&2
  exit 2
fi

echo "FocusFlow execution controls UI smoke passed."
